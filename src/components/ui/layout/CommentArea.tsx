...
const CommentArea: React.FC<CommentAreaProps> = ({comments, currentUser, itemOwner, refreshComments, className}) => {
    ...
    return (
        <div className={`border-t border-b border-blue-3/30 pb-[calc(6vh+2rem+1.5rem)] flex flex-col items-center md:pb-5 md:border-b-2 md:border-t-2 md:border-blue-2/60 md:grow ${className}`}>
            ...
            <div className="md:h-[45vh] flex justify-center md:overflow-y-scroll w-full">
                {commentCount  > 0 ? (
                    ...
                    <p className="text-center w-4/5 text-blue-1 font-medium md:text-xl md:w-full mt-5">Nenhum comentário ainda. Seja o primeiro a comentar!</p>
                )}
            </div>
        </div>
    )
